...
namespace Game1
{
	...
	public class PauseMenu
	{
		...
		public Button continueButton, menuButton;
    ...
    public class GameManager
    {
        ...
        public float SpawnHazardTime = 4;
        public void SpawnHazard()
        {
            
            Random r = new Random();
            GameObject.Side side = GameObject.Side.Left;
            ...
            if (r.Next(0,1) == 0)
            {
                temp = new Shield(gameRef,spriteBatch);
                hazards.Add(temp);
            }
            if (hazard != null)
            hazards.Add(hazard);
        ...
        public bool Collision(Rectangle rect1, Rectangle rect2, float c = .35f)
        {
            ...
            for (int i = 0; i < hazards.Count; i++)
            {
                ...
                else
                {
                    if (Collision((gameRef as Game1).ninja.Rect,hazards[i].Rect))
                    {
                        ...
                        {
                            if ((gameRef as Game1).ninja.currNinjaState == Ninja.NinjaState.Jump && !(hazards[i] is Climber))
                            {
    ...
    public class BirdEnemy : GameObject
    {
        ...
        private static Vector2 RIGHT_POSITION = new Vector2(500 - 36, 0);
    ...
    {
        private static Vector2 LEFT_POSITION = new Vector2(36, -63 + 27);
        private static Vector2 RIGHT_POSITION = new Vector2(500 - 36 - 100, -63 + 27);
        ...
        public Vector2 MoveSpeed = new Vector2(100, Tile.Yspeed);
    ...
    public class Climber : GameObject
    {
        ...
        private static Vector2 LEFT_POSITION = new Vector2(36 - 30, 0);
        private static Vector2 RIGHT_POSITION = new Vector2(500 - 36 - 50, 0);
        ...
        private void ThrowShuriken(int shurikenNum)
        {
            ...
            if (side == Side.Right)
                i = -1;
            (gameRef as Game1).gameManager.hazards.Add(new Shuriken(gameRef, spriteBatchRef, position, climbingSpeed + new Vector2(i * 100, 0)));
            if (shurikenNum > 1)
            { 
                (gameRef as Game1).gameManager.hazards.Add(new Shuriken(gameRef, spriteBatchRef, position, climbingSpeed + new Vector2(i * 100, -50)));
            }
        }
    ...
    public class Ninja
    {
        ...
        public void Draw()
        {
            if ((gameRef as Game1).hasShield)
            {
                if (currNinjaState == NinjaState.Left)
                    spriteBatchRef.Draw(ShieldTexture2D, new Rectangle(rect.X - 23, rect.Y - 6, ShieldTexture2D.Width, ShieldTexture2D.Height), Color.White);
                else if (currNinjaState == NinjaState.Right)
                    spriteBatchRef.Draw(ShieldTexture2D, new Rectangle(rect.X + 12, rect.Y - 6, ShieldTexture2D.Width, ShieldTexture2D.Height), Color.White);
                else
                    spriteBatchRef.Draw(ShieldTexture2D, new Rectangle(rect.X + 15, rect.Y - 6, ShieldTexture2D.Width, ShieldTexture2D.Height), Color.White);
            }
        ...
        public void Update(ref GameState state)
        {
            ...
            if (state == GameState.gameOver && currNinjaState != NinjaState.FallLeft && currNinjaState != NinjaState.FallRight)
            {
                velocity.X = jumpStartVelocity.X / 2f;
                velocity.Y = jumpStartVelocity.Y;
                if (currNinjaState == NinjaState.Left)
                    currNinjaState = NinjaState.FallLeft;
                if (currNinjaState == NinjaState.Right)
    ...
    public class Score
    {
        ...
        {
            spriteBatchRef.Draw(texture2D, rect, Color.White);
            spriteBatchRef.DrawString(font, value.ToString(), new Vector2(25, Game1.HEIGHT - 28), Color.White);
	...
	public class Game1 : Game
    {
		...
		protected override void Initialize()
        {
            ...
            //GraphicsDevice.Viewport = new Viewport(0, 0, 500, 820);
            graphics.PreferredBackBufferHeight = HEIGHT;
            graphics.PreferredBackBufferWidth = WIDTH;
            graphics.ApplyChanges();
           // laundry = new Laundry(this, spriteBatch);
            // TODO: Add your initialization logic here
            ninja = new Ninja(this, spriteBatch);
            //bird = new BirdEnemy(this,spriteBatch,GameObject.Side.Left);
            //climber = new Climber(this, spriteBatch, GameObject.Side.Left, new Vector2(0, 100));
            //squirl = new Squirl(this, spriteBatch, GameObject.Side.Left);
